...
public class Matrix4 implements Matrix<Vector4, Vector4> {
    ...
    @Override
    public float wx() { return m[wx]; }
    @Override
    public float wy() { return m[wy]; }
    @Override
    public float wz() { return m[wz]; }
    @Override
    public float ww() { return m[ww]; }
    @Override
    public final int m() { return 4; }
    @Override
    ...
    public Vector4 row(int i) {
        switch(i) {
            case 0: return new Vector4(m[xx], m[xy], m[xz], m[xw]);
            case 1: return new Vector4(m[yx], m[yy], m[yz], m[yw]);
            case 2: return new Vector4(m[zx], m[zy], m[zz], m[zw]);
            case 3: return new Vector4(m[wx], m[wy], m[wz], m[ww]);
        }
        return null;
    }
    ...
    public Vector4 col(int j) {
        switch(j) {
            case 0: return new Vector4(m[xx], m[yx], m[zx], m[wx]);
            case 1: return new Vector4(m[xy], m[yy], m[zy], m[wy]);
            case 2: return new Vector4(m[xz], m[yz], m[zz], m[wz]);
            case 3: return new Vector4(m[xw], m[yw], m[zw], m[ww]);
        }
        return null;
    }
    ...
    public static class Mutable extends Matrix4 implements Matrix.Mutable<Vector4, Vector4> {
        ...
        @Override
        public float xx(float xx) { return m[Matrix4.xx] = xx; }
        @Override
        public float xy(float xy) { return m[Matrix4.xy] = xy; }
        @Override
        public float xz(float xz) { return m[Matrix4.xz] = xz; }
        @Override
        public float xw(float xw) { return m[Matrix4.xw] = xw; }
        @Override
        public float yx(float yx) { return m[Matrix4.yx] = yx; }
        @Override
        public float yy(float yy) { return m[Matrix4.yy] = yy; }
        @Override
        public float yz(float yz) { return m[Matrix4.yz] = yz; }
        @Override
        public float yw(float yw) { return m[Matrix4.yw] = yw; }
        @Override
        public float zx(float zx) { return m[Matrix4.zx] = zx; }
        @Override
        public float zy(float zy) { return m[Matrix4.zy] = zy; }
        @Override
        public float zz(float zz) { return m[Matrix4.zz] = zz; }
        @Override
        public float zw(float zw) { return m[Matrix4.zw] = zw; }
        @Override
        public float wx(float wx) { return m[Matrix4.wx] = wx; }
        @Override
        public float wy(float wy) { return m[Matrix4.wy] = wy; }
        @Override
        public float wz(float wz) { return m[Matrix4.wz] = wz; }
        @Override
        public float ww(float ww) { return m[Matrix4.ww] = ww; }
        ...
        public Vector4 row(int i, float x, float y, float z, float w) {
            switch(i) {
                case 0: return new Vector4(m[xx] = x, m[xy] = y, m[xz] = z, m[xw] = w);
                case 1: return new Vector4(m[yx] = x, m[yy] = y, m[yz] = z, m[yw] = w);
                case 2: return new Vector4(m[zx] = x, m[zy] = y, m[zz] = z, m[zw] = w);
                case 3: return new Vector4(m[wx] = x, m[wy] = y, m[wz] = z, m[ww] = w);
            }
            return null;
        }
        ...
        public Vector4 col(int j, float x, float y, float z, float w) {
            switch(j) {
                case 0: return new Vector4(m[xx] = x, m[yx] = y, m[zx] = z, m[wx] = w);
                case 1: return new Vector4(m[xy] = x, m[yy] = y, m[zy] = z, m[wy] = w);
                case 2: return new Vector4(m[xz] = x, m[yz] = y, m[zz] = z, m[wz] = w);
                case 3: return new Vector4(m[xw] = x, m[yw] = y, m[zw] = z, m[ww] = w);
            }
            return null;
        }
